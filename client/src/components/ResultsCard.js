...
function ResultsCard(props) {
	...
	for (let i = 0; i < 5; i++) {
		if (i + 1 <= Math.floor(props.biz.googleRating)) {
			googleRatingImage.push(<FontAwesomeIcon icon={faStar} key={'google' + props.biz.yelpID + i}/>)
		} else {
			if (i + 1 > Math.ceil(props.biz.googleRating)){
				googleRatingImage.push(<FontAwesomeIcon icon={faStarReg} key={'google' + props.biz.yelpID + i}/>)
			} else if (props.biz.googleRating - Math.floor(props.biz.googleRating) >= 0.8) {
				googleRatingImage.push(<FontAwesomeIcon icon={faStar} key={'google' + props.biz.yelpID + i}/>)
			} else if(props.biz.googleRating - Math.floor(props.biz.googleRating) >= 0.3) {
					...
					<div className="card-text">
						...
						<div className="jumbotron yelp-data">
							...
							<div className="yelp-review-section">
								...
								<p className="yelp-review-count">{props.biz.yelpReviewCount} reviews</p>
							</div>
						...
						<div className="jumbotron google-data">
							...
							<div className="google-review-section">
								<div className="google-rating">
									<p className="google-rating-num">{props.biz.googleRating}</p>
									{googleRatingImage}
								</div>
