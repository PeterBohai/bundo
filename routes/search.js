...
searchRouter.post('/', (req, res, next) => {
	...
	yelpFusion.businessSearch(searchBody.searchDesc, searchBody.searchLoc, next)
		.then(yelpData =>  {	
		...
		.then(async (filledBizData) => { 
			await Promise.all(filledBizData.businesses.map( async (biz) => {
				...
				await googlePlaces.placeSearch(biz.phone, 'phonenumber')
					.then(placesData => {
						if (placesData.status === 'OK' && placesData.candidates.length >= 0) {
							...
							return googlePlaces.placeSearch(biz.name, 'textquery')
								.then(placesData => {
									if (placesData.status === 'OK' && placesData.candidates.length >= 0) {
										return placesData.candidates[0].place_id
									} else {
					...
					.then(async (googlePlaceId) => {
						...
						await googlePlaces.placeDetail(googlePlaceId)
							.then(detailsData => {
								biz.googleRating = Math.round(detailsData.result.rating * 10) / 10
								...
								if (biz.displayPhone === undefined || biz.displayPhone.length === 0 ) {
									biz.displayPhone = detailsData.result.formatted_phone_number
								}
							})
							.catch(err => {
								biz.googleError = err.message
								logger.error('googlePlaces.placeDetail() error recorded in biz.googleError')
							})
					})
					.catch(err => {
						biz.googleError = err.message
						logger.error('googlePlaces.placeSearch() error recorded in biz.googleError')
					})
							
				// query Facebook Grpahs API (search for id first then get Information data)
				await facebookGraph.pagesSearch(biz.name, biz.latitude, biz.longitude)
					.then(searchData => {
					...
					.then(async (fbPlaceId) => {
						...
						await facebookGraph.placeInfo(fbPlaceId)
							.then(infoData => {
								biz.fbRating = infoData.overall_star_rating ? infoData.overall_star_rating : 'N/A'
								biz.fbReviewCount = infoData.rating_count ? infoData.rating_count : 0
								biz.fbUrl = infoData.link
								logger.debug(biz)
							})
							.catch(err => {
								biz.fbError = err.message
								logger.error('facebookGraph.placeinfo() error recorded in biz.fbError')
							})
					})
					.catch(err => {
						biz.fbError = err.message
						logger.error('facebookGraph.pagesSearch() error recorded in biz.fbError')
					})
			}))
...
searchRouter.post('/details', (req, res) => {
	...
	const fetchYelp = yelpFusion.businessDetailsFromId(ids.yelpID)
		.then(yelpData => {
	...
	const fetchGoogle = googlePlaces.placeDetail(ids.googleID)
		.then(googleData => {
	...
	const fetchFacebook = facebookGraph.placeInfo(ids.facebookID)
		.then(facebookData => {
			bizData.fbRating = facebookData.overall_star_rating ? facebookData.overall_star_rating : 'N/A'
	...
	Promise.all([fetchYelp, fetchGoogle, fetchFacebook])
		.then(() => {
